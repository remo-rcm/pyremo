...
class DataHandler():
  ...
  def __init__(self, sys='SLURM', parallel=False, header_dict={}, \
               job_dir='', log_dir=''):
      self.sys         = sys
  ...
  def _create_job(self,scheduler,jobname,commands,job_dir,log_dir):
      ...
      scheduler.create_job(jobname=jobname,jobscript=jobscript,
  ...
  def _check_file_list(self, file_list, loc):
      ...
      if   loc == 'tape':
        ...
        raise Exception('Unknown location defined in check_archive_files: '+str(loc))
      return check_list
 


  def extract_archive(self, expid, datapath, years=[1979], months=range(1,13), types=[],
                      filters=[], codes=[], loc='', conv=REMO_2015_DISK_ARCHIVE(), destdir='', log_dir='',
                      job_dir='', sc_logfile=''):

      code_filters = []

      if not loc: loc = conv.loc
      if not destdir: destdir = os.getcwd()
      if codes: code_filters=['_c%03d_' % int(code) for code in codes]
     ...
     #               "tar.close()\n"
      ...
      if not self.parallel:
        for year in years:
          file_list  = self.get_file_list(expid, datapath, [year], months, types, conv)
          ...
          for filename in file_list:
            ...
            if code_filters:
              ...
              files_to_extract   = [f for f in files_to_extract if any (c for c in code_filters if c in f )]
            if filters:
              #members_to_extract = [f for f in members_to_extract if any (str(x) for x in filters if str(x) in f.name )]
              files_to_extract = [f for f in files_to_extract if any (str(x) for x in filters if str(x) in f )]
            ...
            for f in files_to_extract: tar.extract(f, path=destdir)
            tar.close()
      else:
        if not sc_logfile: sc_logfile = os.path.join(log_dir,'DataHandler_'+expid+'_extract.jobids.ini')
        sc      = Scheduler(self.sys,name='DataHandler',header_dict=self.header_dict, \
                              logfile=sc_logfile)
        for year in years:
           file_list  = self.get_file_list(expid, datapath, [year], months, types, conv)
           py_command = ''
           for filename in file_list:
             ...
             if filters:
               ...
               files_to_extract = [f for f in files_to_extract if any (str(x) for x in filters if str(x) in f )]
             #logging.info('extracting files: '+str([member.name for member in members_to_extract]))
             py_command += command_tpl.format(files_to_extract, filename, destdir) 
           job_command = JOB_TEMPLATE.format(py_command)
           jobname='DataHandler_extract_'+expid+'_'+str(year)
           self._create_job(sc,jobname=jobname,commands=job_command,\
                            job_dir=job_dir,log_dir=log_dir)
           #sc.submit(jobname)



 
  def retrieve_archive_from_tape(self, expid, datapath, years=[1979], months=range(1,13), types=[],
                                 loc='', conv=REMO_2015_TAPE_ARCHIVE(), destdir='', log_dir='',
                                 job_dir='', sc_logfile=''):
      ...
      if not self.parallel:
        for year in years:
          ...
          # download files
          for filename in file_list:
            download_file('tape',os.path.dirname(filename),
                          os.path.basename(filename),out_dir)
      else:
        if not sc_logfile: sc_logfile = os.path.join(log_dir,'DataHandler_'+expid+'_extract.jobids.ini')
        sc      = Scheduler(self.sys,name='DataHandler',header_dict=self.header_dict, \
                              logfile=sc_logfile)
        for year in years:
           ...
           py_command = ''
           for filename in file_list:
             py_command += command_tpl.format(os.path.dirname(filename),  \
  ...
  def check_archive_files(self, expid, datapath, years=[1979], months=range(1,13), types=[],
      ...
      if missing: 
        ...
        logging.warning('Please note, that the check does not include checks of file size!')
        logging.warning('Also note, that these checks are not fail save, this tool is not responsible.')
  
      return missing
  
  
  def get_file_list(self, expid, datapath='', years=[], months=range(1,13), types=[],
      ...
      if not types: types = conv.known_types  
