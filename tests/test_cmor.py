...
def test_season():
    ...
    assert(pr.cmor.season(cfdt.datetime(2000,1,1)) == 'DJF')
    ...
    bounds = (cfdt.datetime(1999, 12, 1, 0, 0), cfdt.datetime(2000, 3, 1, 0, 0))
    assert(pr.cmor.season_bounds(cfdt.datetime(2000,1,1)) == bounds)
    assert(pr.cmor.mid_of_season(dt.datetime(2000,1,1)) == dt.datetime(2000, 1, 15, 12, 0))
